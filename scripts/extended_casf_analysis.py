...
def _ensure_analyzer_import_path() -> None:
...
def _chembl_map_for_set(ligand_set: str) -> pd.DataFrame:
    _ensure_analyzer_import_path()
    from casf_benchmark.paths import DEFAULT_CHEMBL_MAP_CSV, DEFAULT_CHEMBL_REF_MAP_CSV
    from analyze_casf_conformer_sets import load_chembl_map
...
def _rmsds_to_reference(mols: list[object], reference_mol: object) -> list[float]:
    from analyze_casf_conformer_sets import best_aligned_rmsd
...
def _compute_generation_rmsds(payload: dict[str, object]) -> list[float]:
    _ensure_analyzer_import_path()
    from analyze_casf_conformer_sets import load_casf_ligand, load_sdf
...
def _compute_chembl_pb_rmsds(payload: dict[str, object]) -> list[float]:
    ...
    from analyze_casf_conformer_sets import (
        forcefield_energy,
        get_chembl_mols,
        load_casf_ligand,
    )
...
```bash
PYTHONPATH=src python scripts/extended_casf_analysis.py --dashboard-db {db_path} --output-dir {paths.root}
```
