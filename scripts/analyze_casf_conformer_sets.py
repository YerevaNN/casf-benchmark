...
def _limit_worker_threads() -> None:
    for var in (
        ...
        os.environ[var] = "1"
    if os.environ.get("MOLGEN3D_QUIET_RDKIT_WARNINGS") == "1":
        RDLogger.DisableLog("rdApp.*")


def _configure_rdkit_logging(quiet: bool) -> None:
    if quiet:
        os.environ["MOLGEN3D_QUIET_RDKIT_WARNINGS"] = "1"
        RDLogger.DisableLog("rdApp.*")
...
def _build_ligand_tasks(
    ...
    parts_dir = paths.cache_dir / (
        "geometric_reference_parts" if mode == "reference" else "geometric_generation_parts"
    )
