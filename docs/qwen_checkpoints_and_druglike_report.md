...
| File | Role |
| --- | --- |
| [`src/casf_benchmark/config/generation_runs.yaml`](../src/casf_benchmark/config/generation_runs.yaml) | The manifest: run label → inference output dir, per cohort (`casf16_core`, `casf16_ref`, `druglike`). Runs not evaluated on a cohort simply omit it. Generator-agnostic — `label` is arbitrary, and optional per-entry descriptors (`generator`, `display_label`, …) carry through to the druglike tables — so a non-Qwen method is a new entry plus its CSVs, not a script fork. |
...
| [`tests/test_generation_runs_catalog.py`](../tests/test_generation_runs_catalog.py) | 13 tests: bundled manifest covers all three cohorts, labels unique, CASF16 cohorts ⊆ druglike, config order preserved, error cases, arbitrary non-Qwen labels accepted, label descriptors, and no family claiming a ligand set that has no analysis source. |
| [`tests/test_release_data.py`](../tests/test_release_data.py) | 10 tests (no network): env tag/repo pinning, only the two default DB paths are fetchable, fetch skipped when the file exists or the path is not a default, progress reporting, and no truncated file left after a failed download. |
...
The catalog now says so rather than implying otherwise: the entry's `ref_root`
(`casf_generation_families.yaml`) and its `casf16_ref` cohort (`generation_runs.yaml`)
have been dropped, since a claimed ref root with no analysis tables behind it reads as an
accidental gap and made `run_casf_batch_all.sh` re-fail the item on every sweep. Both
sites record the root cause and what to restore. `tests/test_generation_runs_catalog.py`
now fails if any per-checkpoint family claims a ligand set that has no analysis source.
...
| --- | --- | --- |
| `casf_analysis_dashboard.sqlite` (118 MB), `extended_casf_analysis.sqlite` (95 MB), `casf_per_ligand_long.csv` (129 MB) | Over GitHub's 100 MB blob limit; LFS is disabled for this repo (with LFS off, the filters silently commit pointer stubs instead of data) | GitHub Release assets; gitignored. The dashboard fetches the two SQLite files itself on first run ([`release_data.py`](../src/casf_benchmark/release_data.py), tag pinned by `CASF_DASHBOARD_RELEASE`), so a clone with no data serves the full dashboard. The CSV is a rebuild input the app never opens. |
| `data/results/tables/extended_k_efficiency_failures.csv` | Not a result: ~10k rows of `FileNotFoundError` for generation SDFs absent from the bundled run roots, which ship analysis tables only. Machine-specific absolute paths | Regenerated by any `casf-extended-analysis` run; now gitignored |
...
1. **`qwen_0p6b_4e_from_bigdata_step29600` / ref is still missing.** One degenerate-coordinate
   ...
   this ligand — the same crash will recur on any future run containing it. The catalog
   no longer claims a ref result for it (§3), so this is now a tracked absence rather than
   a silent one. Cheap to finish once the analyzer tolerates it: 1235/1236 ligands are
   already cached under that run root's `analysis/cache`, so the rerun is minutes, not the
   ~3h a cold ref pass takes.
